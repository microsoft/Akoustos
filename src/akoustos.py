...
class Akoustos:
    ...
    def generate_spectrograms(self, spectrogram_duration_in_seconds, clear_spectrograms_space = True, axis = False, sr = 22050, hop_length = 512, fmin = None, x_axis = 'time', y_axis = 'linear', cmap = 'viridis'):
        ...
        Args:
            ...
            axis: binary value of "True" or "False", to indicate whether to show axis on the generated spectrogram.
            sr: number > 0 [scalar]. Sample rate used to determine time scale in x-axis.
            hop_length: int > 0 [scalar]. Hop length, also used to determine time scale in x-axis.
            fmin: float > 0 [scalar] or None. Frequency of the lowest spectrogram bin. Used for Mel and CQT scales.
            x_axis, y_axis: None or str. Range for the x- and y-axes.
            ...
            for complete explanation of all parameters, refer to https://librosa.org/doc/latest/generated/librosa.display.specshow.html.
    ...
    def train_binary_classification_model(self, categories, model_name = 'Customized_CNN', model_version = 'v1', batch_size = 32, optimizer = 'Adam', learning_rate = 0.008, lr_decay = False, num_epochs = 25, train_size = 0.7, val_size = 0.15, test_size = 0.15, by = 'random', include_no_label_category = True):
        ...
        Args:
            ...
            model_version: string. User defined model version, such as "v1".
            batch_size: integer. It represents the number of samples that will be propagated through the network. 
            optimizer: string. It represents which optimizer to use. Possible values include "Adam" and "SGD".
            learning_rate: float. It is a hyperparameter that controls how much to change the model in response to the estimated error each time the model weights are updated.
            lr_decay: binary value of "True" or "False", to indicate whether to use learning rate decay.
            num_epochs: indicates how many times will the input_fn return the whole batch and steps indicates how many times the function should run.
            train_size: float (between 0 and 1.0), representing the percentage of training data. The sum of "train_size", "val_size" and "test_size" needs to be 1.
            val_size: float (between 0 and 1.0), representing the percentage of training data. The sum of "train_size", "val_size" and "test_size" needs to be 1.
            test_size: float (between 0 and 1.0), representing the percentage of training data. The sum of "train_size", "val_size" and "test_size" needs to be 1.
            by: way of splitting data. Possible values are "random" and "order". If "random", then split data by random; if "order", then split data according to the alphabetical order of audio filenames
